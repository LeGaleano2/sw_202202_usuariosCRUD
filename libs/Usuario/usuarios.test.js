...
describe('Testing Usuarios CRUD', ()=>{
  ...
  beforeAll(async ()=>{
    ...
    await Usr.addUsuario(
      {  nombre : "Test 1", correo : "test1@prueba.com", contra : "nondisclose1", avatar : "testAvatar1",  estado : "ACT" }
    );
    await Usr.addUsuario(
      { nombre : "Test 2", correo : "test2@prueba.com", contra : "nondisclose2", avatar : "testAvatar2",  estado : "ACT" }
    );
    await Usr.addUsuario(
      { nombre : "Test 3", correo : "test3@prueba.com", contra : "nondisclose3", avatar : "testAvatar3",  estado : "ACT"}
    );
    return true;
  });
  afterAll(async ()=>{
  ...
  test('Usuarios getAll Records', async ()=>{
    const results = await Usr.getUsuario();
  ...
  test('Usuarios updateOne Record', async ()=>{
    ...
    const updatedRecord = await Usr.updateUsuario({
      codigo: 2,
      nombre: "Test 2 UPD",
      correo: "test2_2@prueba.com",
      contra: "nondisclose2_2",
